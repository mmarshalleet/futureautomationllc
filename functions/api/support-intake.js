...
// Creates a support ticket, logs it, and routes the user to payment.
//
// Payment options supported:
// - PayPal Checkout (recommended): PAYPAL_CLIENT_ID + PAYPAL_CLIENT_SECRET
// - PayPal "Buy Now" link (legacy): PAYPAL_BUSINESS (merchant email or merchant id)
...
async function logToSheets(env, record) {
  // Optional: set SHEETS_WEBHOOK_URL + SHEETS_WEBHOOK_KEY
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const ticketId = uid();
    const isPlantDown =
      String(payload.plantDown || payload.type || "").toLowerCase().includes("true") ||
      String(payload.type || "").toLowerCase().includes("plant");

    const origin = getSiteOrigin(request);

    const hasPayPalCheckout = !!(env.PAYPAL_CLIENT_ID && env.PAYPAL_CLIENT_SECRET);
    ...
    if (hasPayPalCheckout) {
      // Modern PayPal Checkout flow (Smart Buttons -> create/capture via API endpoints)
      checkoutUrl = `${origin}/support-checkout.html?ticket=${encodeURIComponent(ticketId)}&plantDown=${
        isPlantDown ? "1" : "0"
      }`;
      paymentFlow = "paypal_checkout";
    } else {
      // Legacy PayPal "Buy Now" link flow
      ...
      paypalUrl = buildPayPalOneTimeUrl({
        business: env.PAYPAL_BUSINESS,
        itemName: isPlantDown ? "Plant-Down Remote Support (Initial Incident)" : "Remote Support (Initial Incident)",
    ...
    return new Response(JSON.stringify({ ok: true, ticketId, checkoutUrl, paypalUrl }), {
      status: 200,
      headers: { "content-type": "application/json; charset=utf-8" },
    });
  } catch (err) {
